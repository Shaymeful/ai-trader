...
print("Features:")
print(
    f"  Trade Rationale:        {'ON' if config['effective']['trade_rationale']['enabled'] else 'OFF'} (source: {config['sources']['trade_rationale']['enabled']})"
)
print(
    f"  Daily Journal:          {'ON' if config['effective']['daily_journal']['enabled'] else 'OFF'} (source: {config['sources']['daily_journal']['enabled']})"
)
print(
    f"  Strategy Critique:      {'ON' if config['effective']['strategy_critique']['enabled'] else 'OFF'} (source: {config['sources']['strategy_critique']['enabled']})"
)
print(
    f"  Sector Recommendations: {'ON' if config['effective']['sector_recommendations']['enabled'] else 'OFF'} (source: {config['sources']['sector_recommendations']['enabled']})"
)
print()
print("=" * 60)
