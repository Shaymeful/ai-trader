...
def test_dry_run_does_not_submit_orders(clean_env, monkeypatch, tmp_path):
    ...
    with patch("src.app.__main__.MockBroker") as mock_broker_class:
        ...
        with patch("sys.argv", ["ai-trader", "--mode", "dry-run", "--dry-run", "--once"]), contextlib.suppress(SystemExit):
            main()
...
def test_dry_run_shows_decisions(clean_env, monkeypatch, tmp_path, capsys):
    ...
    with patch("sys.argv", ["ai-trader", "--mode", "dry-run", "--dry-run", "--once", "--symbols", "AAPL,MSFT"]), contextlib.suppress(SystemExit):
        main()
...
def test_dry_run_with_symbols(clean_env, monkeypatch, tmp_path, capsys):
    ...
    with patch("sys.argv", ["ai-trader", "--mode", "dry-run", "--dry-run", "--once", "--symbols", "AAPL"]), contextlib.suppress(SystemExit):
        main()
