...
def test_setup_outputs_creates_trades_csv(temp_dir, test_run_id):
    ...
    # Check header is present (with cost tracking fields and strategy attribution)
    with open(csv_path) as f:
        header = f.readline().strip()
        assert (
            header
            == "timestamp,symbol,side,quantity,price,order_id,client_order_id,run_id,reason,expected_price,slippage_abs,slippage_bps,spread_bps_at_submit,strategy_id,strategy_version"
        )
...
def test_trades_csv_empty_if_no_signals(temp_dir, monkeypatch):
    ...
    # But at minimum, we should have the header (with cost tracking fields and strategy attribution)
    assert len(lines) >= 1
    assert (
        lines[0].strip()
        == "timestamp,symbol,side,quantity,price,order_id,client_order_id,run_id,reason,expected_price,slippage_abs,slippage_bps,spread_bps_at_submit,strategy_id,strategy_version"
